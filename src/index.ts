...
if (args.includes("--d")) {
  ...
  description = args.slice(descriptionIndex + 1).join(" ");
  title = args.slice(0, descriptionIndex).join(" ");
} else {
  description = null;
  title = args.join(" ");
}
...
//   if (fs.existsSync(path.join(path.normalize(), "RetrievalKanban", "AllItems")))
//     await fsp.writeFile(
//       path.join(
//         path.normalize(
//           "/Users/nickolas.shtayn/Library/Mobile Documents/com~apple~CloudDocs"
